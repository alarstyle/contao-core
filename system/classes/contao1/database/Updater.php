...
class Updater extends Controller
{
	...
	public function run300Update()
	{
		...
		foreach (scan(TL_ROOT . '/system/plugins') as $strFolder)
		{
			if (is_dir(TL_ROOT . '/system/plugins/' . $strFolder) && is_dir(TL_ROOT . '/system/plugins/' . $strFolder . '/html'))
			{
	...
	public function updateFileTreeFields()
	{
		...
		foreach (scan(TL_ROOT . '/system/plugins') as $strModule)
		{
	 ...
	 */
	protected function createContentElement(\Contao\Database\Result $objElement, $strPtable, $strField)
	{
		$set = array
		(
			'pid'         => $objElement->id,
