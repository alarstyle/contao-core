 ...
 */
class RadioTable extends \Contao\Editor
{
    ...
    <tr>';
			...
			for ($j=($i*$this->intCols); $j<(($i+1)*$this->intCols); $j++)
			{
				...
				if (strlen($value))
				{
					...
					$return .= '
      <td><input type="radio" name="'.$this->strName.'" id="'.$this->strName.'_'.$i.'_'.$j.'" class="tl_radio" value="'.specialchars($value).'" onfocus=""'.$this->isChecked($this->arrOptions[$j]).$this->getAttributes().'> <label for="'.$this->strName.'_'.$i.'_'.$j.'">'.$label.'</label></td>';
				}
