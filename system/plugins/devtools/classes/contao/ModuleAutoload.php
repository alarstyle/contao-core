...
class ModuleAutoload extends \Contao\BackendModule
{
	...
	protected function compile()
	{
		...
		// List all modules
		foreach (scan(TL_ROOT . '/system/plugins') as $strFile)
		{
			if (strncmp($strFile, '.', 1) === 0 || !is_dir(TL_ROOT . '/system/plugins/' . $strFile))
			{
				continue;
			}
	...
	protected function createAutoloadFiles()
	{
		...
		foreach ($arrModules as $strModule)
		{
			...
			$arrDefaultConfig = array_merge($arrDefaultConfig, parse_ini_file(TL_ROOT . '/system/plugins/' . $strModule . '/config/autoload.ini', true));

			/** @var \SplFileInfo[] $objFiles */
			$objFiles = new \RecursiveIteratorIterator(
				new \RecursiveDirectoryIterator(
					TL_ROOT . '/system/plugins/' . $strModule,
			...
			foreach ($objFiles as $objFile)
			{
				...
				{
					$strRelpath = str_replace(TL_ROOT . '/system/plugins/' . $strModule . '/', '', $objFile->getPathname());
			...
			foreach ($arrFiles as $strFile)
			{
				...
				$size = filesize(TL_ROOT . '/system/plugins/' . $strModule . '/' . $strFile);
				$fh = fopen(TL_ROOT . '/system/plugins/' . $strModule . '/' . $strFile, 'rb');
				...
				if ($arrConfig['register_classes'])
				{
					...
					$arrClassLoader[$strKey] = 'system/plugins/' . $strModule . '/' . $strFile;
			...
			// Scan for templates
			if (is_dir(TL_ROOT . '/system/plugins/' . $strModule . '/templates'))
			{
				/** @var \SplFileInfo[] $objFiles */
				$objFiles = new \RecursiveIteratorIterator(
					new \RecursiveDirectoryIterator(
...
EOT
				...
				{
					$strRelpath = str_replace('system/plugins/' . $strModule . '/', '', $strPath);
