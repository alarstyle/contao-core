...
class ModuleLabels extends BackendModule
{
	...
	protected function compile()
	{
		...
		// Get languages
		foreach (scan(TL_ROOT . '/system/plugins/core/languages') as $strLanguage)
		{
		...
		if (\Input::post('FORM_SUBMIT') == 'tl_labels')
		{
			...
			foreach (scan(TL_ROOT . '/system/plugins') as $strDir)
			{
				$strPath = 'system/plugins/' . $strDir . '/languages/en';
				$strLang = 'system/plugins/' . $strDir . '/languages/' . $lng;
