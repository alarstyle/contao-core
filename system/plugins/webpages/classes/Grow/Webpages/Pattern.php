...
class Pattern extends Controller
{
    ...
    {
        return static::$table === \Contao\Models\ContentModel::getTable();
    }
    ...
    public function initializeSystemHook()
    {
        ...
        foreach( $templates as $templateName )
        {
            ...
            {
                $GLOBALS['TL_CTE'][static::$templatesData[$templateName]['contentCategory'] ?: 'patterns'][$templateName] = 'Grow\\Webpages\\Elements\\ContentPattern';
            }
            if (!static::$templatesData[$templateName]['disableModule'])
            {
                $GLOBALS['FE_MOD'][static::$templatesData[$templateName]['moduleCategory'] ?: 'patterns'][$templateName] = 'Grow\\Webpages\\Modules\\ModulePattern';
            }
        }
        ...
        static::$isEnabled = ($this->isContent() || $this->isModule()) && (Input::get('act') === 'edit' || TL_SCRIPT === 'contao/file.php' || TL_SCRIPT === 'contao/page.php' || TL_SCRIPT === 'system/modules/picker/public/picker.php' || TL_SCRIPT === 'system/modules/filemanager/public/filemanager.php');
    ...
    public function loadDataContainerHook($strTable)
    {
        ...
        foreach ($patternFields as $fieldName=>$objVar)
        {
            ...
            else
            {
                ...
                $GLOBALS['TL_DCA'][static::$table]['fields'][$fieldName]['save_callback'][] = array('Grow\\Webpages\\Pattern', 'preventFieldSaving');
                $GLOBALS['TL_DCA'][static::$table]['fields'][$fieldName]['load_callback'][] = array('Grow\\Webpages\\Pattern', 'setVariable');
                $strPalettePart .= ',' . $fieldName;
            }
        }
        ...
        if ($this->isContent())
        {
            $strPalette = '{type_legend},type' . $strPalettePart;
        }
        else
        {
            $strPalette = '{title_legend},name,type' . $strPalettePart;
        }

        $GLOBALS['TL_DCA'][static::$table]['palettes'][$patternName] = $strPalette;

        // HOOK: allow to change pattern data
        if (isset($GLOBALS['TL_HOOKS']['addPatternToDataContainer']) && is_array($GLOBALS['TL_HOOKS']['addPatternToDataContainer']))
        {
            foreach ($GLOBALS['TL_HOOKS']['addPatternToDataContainer'] as $callback)
            {
                $this->import($callback[0]);
                $this->$callback[0]->$callback[1]($strTable);
            }
        }
    }
    ...
    public function saveVariables($dc)
    {
        ...
        foreach (static::$templatesData[$dc->activeRecord->type]['fields'] as $strVarName=>$objVariable)
        {
            ...
            switch ($objVariable['inputType'])
            {
                ...
                case 'file':
                    $strDbField = 'file';
                    $newValue = StringUtil::uuidToBin($newValue);
                    break;
    ...
    public function saveData($value, $dc)
    {
        ...
        foreach ($GLOBALS['TL_DCA'][static::$table]['fields'] as $fieldName=>$fieldDCA)
        {
            ...
            switch ($fieldDCA['inputTypeOriginal'])
            {
                ...
                case 'image':
                case 'file':
                    $newFieldValue = StringUtil::uuidToBin($newFieldValue);
                    break;
