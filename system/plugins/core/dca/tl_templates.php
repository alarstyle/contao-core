	...
	(
		...
		(
			...
			(
				...
				'href'                => 'act=delete',
				'icon'                => 'delete.gif',
				'attributes'          => 'onclick="if(!confirm(\'' . $GLOBALS['TL_LANG']['MSC']['deleteConfirmFile'] . '\'))return false;"'
			),
			'source' => array
			(
  ...
  <li>' . implode(' &gt; </li><li>', $arrLinks) . '</li>
	...
	public function addNewTemplate()
	{
		...
		if (Input::post('FORM_SUBMIT') == 'tl_create_template')
		{
			...
			if (strncmp($strOriginal, 'system/plugins/', 15) !== 0 || !file_exists(TL_ROOT . '/' . $strOriginal))
			{
		...
		foreach (PluginLoader::getActive() as $strModule)
		{
			...
			if ($strModule == 'repository' || !is_dir(TL_ROOT . '/system/plugins/' . $strModule . '/templates'))
			{
				continue;
			}
