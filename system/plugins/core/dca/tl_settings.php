...
class tl_settings extends \Contao\Backend
{
	...
	{
		$arrModules = array();
		$arrFolders = scan(TL_ROOT . '/system/plugins');
		...
		foreach ($arrFolders as $strFolder)
		{
			...
			if ($strFolder == 'core' || !is_dir(TL_ROOT . '/system/plugins/' . $strFolder))
			{
				continue;
			}

			$arrModules[$strFolder] = !file_exists(TL_ROOT . '/system/plugins/' . $strFolder . '/.skip');
		}
  ...
  <fieldset id="ctrl_' . $dc->field . '" class="tl_checkbox_container">
    ...
    <label for="check_all_' . $dc->inputName . '" style="color:#a6a6a6"><em>' . $GLOBALS['TL_LANG']['MSC']['selectAll'] . '</em></label><br>';
		...
		foreach ($arrModules as $strModule=>$blnActive)
		{
			if (!$blnActive)
			{
				$strFile = 'system/plugins/' . $strModule . '/languages/' . $lng . '/modules';
			...
			$return .= '
    <input type="checkbox" name="' . $dc->inputName . '[]" id="opt_' . $dc->inputName . '_' . $i . '" class="tl_checkbox" value="' . $strModule . '" onfocus=""' . ($blnActive ? '' : ' checked') . '>
