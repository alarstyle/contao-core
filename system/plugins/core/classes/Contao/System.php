...
abstract class System
{
	...
	public static function loadLanguageFile($strName, $strLanguage=null, $blnNoCache=false)
	{
		...
		foreach ($arrCreateLangs as $strCreateLang)
		{
			...
			else
			{
				...
				{
					$strFile = 'system/plugins/' . $strModule . '/languages/' . $strCreateLang . '/' . $strName;
	...
	public static function isInstalledLanguage($strLanguage)
	{
		...
		{
			static::$arrLanguages[$strLanguage] = (is_dir(TL_ROOT . '/system/plugins/core/languages/' . $strLanguage) || is_dir(TL_ROOT . '/system/cache/language/' . $strLanguage) || in_array($strLanguage, array_unique(array_map('basename', glob(TL_ROOT . '/system/plugins/*/languages/*')))));
		}
    ...
    public static function getFlags()
    {
        ...
        foreach ($countries as $key => $name) {
            $countries[$key] = '<span class="flag flag-' . $key . '"></span>';
        }
	...
	public static function getLanguages($blnInstalledOnly=false)
	{
		...
		asort($arrAux);
		$arrBackendLanguages = scan(TL_ROOT . '/system/plugins/core/languages');
